...
void reset_histograms()
{
  ...
  histI_ptLL->Reset();
  histI_dPhi_LL->Reset();
  histI_dPhiLLmet->Reset();
  histI_mt->Reset();

  //reset Leading Lepton histograms
  hist_leadleptpt->Reset();
  hist_leadlepteta->Reset();
  hist_leadleptE->Reset();
  hist_leadleptphi->Reset();
  hist_leadleptch->Reset();
  hist_leadleptID->Reset();
  hist_leadlept_ptc->Reset();
  hist_leadleptetc->Reset();
  hist_leadlepz0->Reset();
  hist_leadlepd0->Reset();

  //reset Subleading Lepton histograms
  hist_subleadleptpt->Reset();
  hist_subleadlepteta->Reset();
  hist_subleadleptE->Reset();
  hist_subleadleptphi->Reset();
  hist_subleadleptch->Reset();
  hist_subleadleptID->Reset();
  hist_subleadlept_ptc->Reset();
  hist_subleadleptetc->Reset();
  hist_subleadlepz0->Reset();
  hist_subleadlepd0->Reset();

  //reset Jet variables histograms
  hist_n_jets->Reset();
  hist_n_bjets->Reset();
...
void FillHistogramsSubleadlept( double m, float w , TString s)
{
